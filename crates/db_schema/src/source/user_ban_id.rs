...
impl BanId {
  pub async fn create(conn: &mut DbConn<'_>) -> Result<Self, Error> {
    insert_into(ban_id)
      .default_values()
  ...
  pub async fn read(conn: &mut DbConn<'_>, ban_id_val: Uuid) -> Result<Self, Error> {
    ban_id.find(ban_id_val).first::<Self>(conn).await
  }

  pub async fn read_opt(conn: &mut DbConn<'_>, ban_id_val: Uuid) -> Option<Self> {
...
impl UserBanId {
  ...
  pub async fn get_by_user(conn: &mut DbConn<'_>, user_id_val: &i32) -> Option<Self> {
    let a = user_ban_id
  ...
  pub async fn get_all_by_user(conn: &mut DbConn<'_>, user_id_val: &i32) -> Option<Vec<Self>> {
    let a = user_ban_id
      .filter(uid.eq(user_id_val))
      .load::<Self>(conn)
      .await;
