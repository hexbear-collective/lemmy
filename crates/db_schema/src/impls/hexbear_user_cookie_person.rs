...
impl HexbearUserCookiePerson {
  pub async fn process_cookie(
    ...
    let incoming_uuid = uuid::Uuid::parse_str(&cookie_val).unwrap_or(Uuid::default());
    let incoming_uuid_cookies = Self::get_users_by_uuid(pool, incoming_uuid).await.unwrap();
    let logged_in_users_cookies = Self::get_by_person_id(pool, person_id).await.unwrap();

    let incoming_users = incoming_uuid_cookies
      .into_iter()
      .map(|f| f.person_id)
      .collect::<Vec<PersonId>>();
    if incoming_users.contains(&person_id) {
      return cookie_val; // incoming uuid is valid and already assigned to current logged in user.
    }

    if incoming_uuid == Uuid::default() && logged_in_users_cookies.is_empty() {
      return "".to_string(); //no incoming cookie and no cookies stored for user
    }
    if incoming_uuid == Uuid::default() && !logged_in_users_cookies.is_empty() {
      return logged_in_users_cookies
        .into_iter()
        .nth(0)
        .unwrap()
        .cookie_uuid
    ...
    //incoming cookie has valid uuid, associate with person logging in
    Self::associate(pool, person_id, incoming_uuid).await;
    return incoming_uuid.to_string();
  }
