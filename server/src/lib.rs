...
lazy_static! {
  ...
  static ref SLUR_REGEX: Regex = RegexBuilder::new(r"([^\p{P}\s]*?(f(a|4)g(got|g)?){1,}|maricos?|(n(i|1)g(\b|g?(a|er)?(s|z)?)){1,}\b|dindu(s?){1,}|mudslime?s?|(k(i|y)kes?){1,}|(mongoloids?){1,}|(towel\s*heads?){1,}|\bspi(c|k)s?\b|(spi(c|k)s){2,}|\bchinks?|(chinks?){1,}|(niglets?){1,}|beaners?|\bjaps?\b|(japs){2,}|\bcoons?\b|(coons?){2,}|(jungle\s*bunn(y|ies?)){1,}|(jigg?aboo?s?){1,}|\bpakis?\b|(pakis?){2,}|(rag\s*heads?){1,}|(gooks?){1,}|(cunt(s|boy)?){1,}|(feminazis?){1,}|(whor(es?|ing)){1,}|\bslut(s|t?y)?|(slut(s|t?y)){2,}|\btr(a|@)nn?(y|ies?)|(tr(a|@)nn?(y|ies?)){1,}|(ladyboy(s?)){1,}|[^\p{P}\s]*?tard(ed|s)?\b|(tard){2,}|(hymie){1,}|(porch\s?monkey){1,}|(zh(y|i)d(ovka)?){1,}|\bching\s?chong\b|(ching\s?chong\s?){1,}|(chong\s?ching\s?){1,}|(hefemale){1,}|(dickgirl){1,}|(hermie){1,}|(\babb?o\b)|(abb?o){2,}|(boong){1,})").case_insensitive(true).build().unwrap();
