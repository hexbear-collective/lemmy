...
AS $BODY$
begin
  insert into chapo.user_stat (user_id) values (NEW.id);
  return null;
end $BODY$;
...
insert into chapo.user_stat (user_id, number_of_comments, number_of_posts, post_score, comment_score)
...
CREATE OR REPLACE VIEW chapo.user_view
 AS
 SELECT 
    u.id,
    u.actor_id,
  ...
  FROM user_ u
  LEFT JOIN chapo.user_stat s on u.id = s.user_id;
